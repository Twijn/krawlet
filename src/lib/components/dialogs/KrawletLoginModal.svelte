<script lang="ts">
    ...
    function normalizeQuickCode(value: string) {
        quickCode = value.toUpperCase().replace(/[^0-9]/g, '').slice(0, 6);
    }
...
<Modal {open} {onSubmit} {onClose} title="Krawlet Login" maxWidth="500px">
    <Alert noMargin variant="info">Use <code>\krawlet api</code> on ReconnectedCC to generate a quick code.</Alert>
