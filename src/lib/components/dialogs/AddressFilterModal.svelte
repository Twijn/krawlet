<script lang="ts">
    ...
    import { faWallet, faStore, faBuilding, faDice, faCheck, faUser } from "@fortawesome/free-solid-svg-icons";
    ...
    let filteredKnown: KnownAddress[] = $derived(
        filteredAddresses.filter((a): a is KnownAddress => "id" in a && !("private" in a) && !("minecraftUUID" in a)).slice(0, 5)
    );
