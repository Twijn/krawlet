<script lang="ts">
	...
	const isDefault = $derived(sortedColumn === defaultColumn && sortDirection === defaultDirection);

	const columnLabel = $derived(columns.find((c) => c.key === sortedColumn)?.label || sortedColumn);
</script>

{#if !isDefault && sortedColumn}
	<Chip label="Sort" value="{columnLabel} {sortDirection}" onRemove={onReset} />
{/if}
