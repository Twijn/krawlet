<script lang="ts">
	...
	const canSubmit = $derived<boolean>(
		selectedOption === 'purchase'
			? privatekey.trim() !== '' && normalizedPurchaseName !== ''
			: selectedOption === 'transfer'
				? privatekey.trim() !== '' && selectedName.trim() !== '' && toAddress.trim().length === 10
				: selectedOption === 'update'
					? privatekey.trim() !== '' && selectedName.trim() !== ''
					: false
	);
...
<Section lgCols={6} mdCols={12}>
	...
	<AddressSelector mode="privatekey" bind:privatekey bind:address label={$t$('address.address')} />
	...
	{#if selectedOption === 'transfer'}
		<AddressSelector mode="address" label={$t$('transaction.recipient')} bind:address={toAddress} />
