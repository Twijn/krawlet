            ...
            <div class="shop">
                ...
                <div class="shop-body table-container">
                    <table>
                        <tbody>
                            <tr>
                                <th>Total Items</th>
                                <td class="right">{items.length} <small>listing{items.length === 1 ? "" : "s"}</small></td>
                            </tr>
                            <tr>
                                <th>Total Stock</th>
                                <td class="right">{totalStock.toLocaleString()} <small>item{totalStock === 1 ? "" : "s"}</small></td>
                            </tr>
                            {#if shop.addresses}
                ...
                <div class="buttons">
                    {#if shop.locationCoordinates && shop.locationCoordinates.split(" ").length === 3}
                        ...
                        <Button variant="secondary" newTab={true} full={true} href="https://map.reconnected.cc/#world:{coordinates[0]}:{coordinates[1]}:{coordinates[2]}:40:0:0:0:1:flat">
                            Locate Shop
                        </Button>
                    {/if}
                    <Button variant="primary" full={true} href="/shops/{shop.id}">
                        View Shop
                    </Button>
                </div>
            </div>
