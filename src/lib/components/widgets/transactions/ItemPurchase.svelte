<script lang="ts">
	...
	function send(e: Event) {
		...
		if (!privatekey) {
			...
			confirm.confirm({
				message: `Are you sure you want to send ${amount.value.toFixed(2)} KRO to ${to}?`,
				...
				confirm: async () => {
					...
					try {
						...
						balances[to.value] = balances[to.value] + amount.value;
...
{#if to.value && to.value.length === 10 && typeof(balances[to.value]) === "number"}
