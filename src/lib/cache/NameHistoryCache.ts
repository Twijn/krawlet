...
export class NameHistoryCache extends KromerCache<NameHistoryLookupQuery, NameHistoryCacheResult> {
    ...
    protected async fetch(params: NameHistoryLookupQuery): Promise<NameHistoryCacheResult | null> {
        const response = await kromer.names[params.type === "history" ? 'lookupNameHistory' : 'lookupNameTransactions'](params.name, {
    ...
    private filterAndSort(
        ...
        if (params.type === 'history') {
            result = result.filter((tx) => ['name_purchase', 'name_a_record', 'name_transfer'].includes(tx.type));
