<script lang="ts">
	...
	let decodedAddress = $derived(
			pkey.length === 0
					? ""
					: kromer.addresses.decodeAddressFromPrivateKey(pkey)
	);
...
<div class="col-12">
	<Alert variant="danger">
		...
		<p>
			Wallets are still in beta! Make sure you back up your keys elsewhere!
		</p>
	</Alert>
</div>
...
<Section lgCols={4} mdCols={12}>
	...
	<form method="POST">
		<label>
			...
			<small>{#if $walletStore.wallets.length > 0}You must use the same password as your past wallets!{/if} Make sure you keep this safe!</small>
		</label>
		<label>
			Name
		...
		{#if decodedAddress.length === 10}
			<p>
				<strong>Decoded Address:</strong> {decodedAddress}
			</p>
		{/if}
		<Button type="submit" full={true} onClick={addWallet}>
			Add Wallet
		</Button>
	</form>
</Section>
